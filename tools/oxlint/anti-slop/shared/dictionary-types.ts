...
/**
 * View a heritage clause as the type reference it names, so `interface X extends Record<K, V>`
 * resolves like `type X = Record<K, V>`. `parent` is kept so lexical shadowing checks still walk
 * a real scope chain.
 */
export function heritageTypeReference(
...
function dictionaryValueTypes(
	...
	if (interfaceDeclarations !== undefined) {
		...
		return interfaceDeclarations.flatMap((declaration) => [
			...declaration.body.body.flatMap((member): readonly ResolvedType[] =>
				member.type === "TSIndexSignature" && member.typeAnnotation !== null
					? [{ type: member.typeAnnotation.typeAnnotation, substitutions }]
					: [],
			),
			...declaration.extends.flatMap((heritage) =>
				dictionaryValueTypes(
					heritageTypeReference(heritage),
					environment,
					substitutions,
					nextResolving,
				),
...
export function classifyWideningTarget(
	...
	const interfaceDeclarations = environment.interfaces.get(name);
	if (interfaceDeclarations !== undefined) {
		return interfaceWideningTarget(name, interfaceDeclarations, environment, new Set());
	}
	const alias = visibleTypeAlias(name, unwrapped, environment.typeAliases);
...
/**
 * An interface is only as narrow as what it inherits. `interface X extends Record<string, V> {}`
 * is the same open dictionary as the alias form, so it must classify the same way.
 */
function interfaceWideningTarget(
	...
	for (const declaration of declarations) {
		if (declaration.body.body.some((member) => member.type === "TSIndexSignature")) {
			return { kind: "open dictionary" };
		}
		for (const heritage of declaration.extends) {
			const reference = heritageTypeReference(heritage);
			const heritageName = typeReferenceName(reference);
			const inherited =
				heritageName === null ? null : (environment.interfaces.get(heritageName) ?? null);
			const target =
				heritageName === null || inherited === null
					? classifyWideningTarget(reference, environment)
					: interfaceWideningTarget(heritageName, inherited, environment, nextResolving);
